...
def test():
    '''Run tests suite'''
    lrun('nosetests --rednose --force-color udata_gouvlu', pty=True)
...
def cover():
    ...
    lrun('nosetests --rednose --force-color --with-coverage '
         '--cover-html --cover-package=udata_gouvlu', pty=True)


@task
def qa():
    '''Run a quality report'''
    lrun('flake8 udata_gouvlu')
