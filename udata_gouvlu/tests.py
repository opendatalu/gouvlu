...
class gouvluThemeTest(FrontTestCase):
    '''Ensure themed views render'''
    settings = gouvluSettings
...
class gouvluMetricsTest(DBTestMixin, TestCase):
    '''Check metrics'''
    settings = gouvluSettings
...
class SpecificUrlsTest(FrontTestCase):
    ...
    def test_redevances(self):
        response = self.client.get(url_for('gouvlu.redevances'))
        self.assert200(response)

    def test_faq_home(self):
        response = self.client.get(url_for('gouvlu.faq'))
        self.assert200(response)
        self.assert_template_used('faq/home.html')

    def test_citizen_faq(self):
        response = self.client.get(url_for('gouvlu.faq', section='citizen'))
        self.assert200(response)
        self.assert_template_used('faq/citizen.html')

    def test_producer_faq(self):
        response = self.client.get(url_for('gouvlu.faq', section='producer'))
        self.assert200(response)
        self.assert_template_used('faq/producer.html')

    def test_reuser_faq(self):
        response = self.client.get(url_for('gouvlu.faq', section='reuser'))
        self.assert200(response)
        self.assert_template_used('faq/reuser.html')

    def test_developer_faq(self):
        response = self.client.get(url_for('gouvlu.faq', section='developer'))
        self.assert200(response)
        self.assert_template_used('faq/developer.html')

    def test_third_party_faq(self):
        response = self.client.get(
            url_for('gouvlu.faq', section='system-integrator'))
        self.assert200(response)
        self.assert_template_used('faq/system-integrator.html')

    def test_terms(self):
        response = self.client.get(url_for('gouvlu.terms'))
        self.assert200(response)

    def test_credits(self):
        response = self.client.get(url_for('gouvlu.credits'))
        self.assert200(response)
...
class DataconnexionsTest(FrontTestCase):
    ...
    def test_redirect_to_last_dataconnexions(self):
        response = self.client.get(url_for('gouvlu.dataconnexions'))
        self.assertRedirects(response, url_for('gouvlu.dataconnexions6'))

    def test_render_dataconnexions_5_without_data(self):
        response = self.client.get(url_for('gouvlu.dataconnexions5'))
        self.assert200(response)
    ...
    def test_render_dataconnexions_6_without_data(self):
        response = self.client.get(url_for('gouvlu.dataconnexions6'))
        self.assert200(response)
...
class C3Test(FrontTestCase):
    ...
    def test_redirect_c3(self):
        response = self.client.get(url_for('gouvlu.c3'))
        self.assertRedirects(response, '/en/climate-change-challenge')

    def test_render_c3_without_data(self):
        response = self.client.get(url_for('gouvlu.climate_change_challenge'))
        self.assert200(response)


class TerritoriesSettings(gouvluSettings):
...
class TerritoriesTest(FrontTestCase):
    ...
    def test_with_gouvlu_territory_datasets(self):
        bdr = GeoZoneFactory(
...
class SitemapTest(FrontTestCase):
    ...
    def test_urls_within_sitemap(self):
        ...
        urls = [
            url_for('gouvlu.credits_redirect', _external=True),
            url_for('gouvlu.terms_redirect', _external=True),
            url_for('gouvlu.redevances_redirect', _external=True),
            url_for('gouvlu.faq_redirect', _external=True),
        ]
        for section in ('citizen', 'producer', 'reuser', 'developer',
                        'system-integrator'):
            urls.append(url_for('gouvlu.faq_redirect',
